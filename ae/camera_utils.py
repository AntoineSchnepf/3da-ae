...
from datasets.multiview_dataset import DatasetBank
...
class PoseSampler: 
    ...
    def _get_extrinsics_from_batched_elev_azi(elevations, azimuths, radius) : 
        ...
        camera_origins[:, 0] = radius*torch.sin(torch.pi/2 - elevations) * torch.cos(azimuths)
        camera_origins[:, 1] = radius*torch.sin(torch.pi/2 - elevations) * torch.sin(azimuths)
        camera_origins[:, 2] = radius*torch.cos(torch.pi/2 - elevations)
...
class LazyPoseSampler(PoseSampler):
    ...
    def __init__(self, dataset_name:str):
        metadata = DatasetBank.get_metadata(dataset_name)
