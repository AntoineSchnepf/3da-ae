...
class TriPlaneRenderer(torch.nn.Module):
    def __init__(self,                   
        ...
        super().__init__()
        self.do_concatenate = do_concatenate
        self.n_main_channels = n_main_channels
        self.n_concat_channels = n_concat_channels
        
        self.total_channels=n_main_channels
        if self.do_concatenate: 
            self.total_channels += n_concat_channels
            
        self.n_features = n_features
        self.renderer = ImportanceRenderer()
        self.ray_sampler = RaySampler()
        self.decoder = OSGDecoder(n_features, self.total_channels, aggregation_mode, use_coordinates, use_directions, positionnal_encoding_kwargs)
    ...
    def synthesis(self, planes, pose, neural_rendering_resolution=None):
        ...
        res = {
            'img': feature_image[:, :, :self.n_main_channels], 
            'img_depth': depth_image
        }  
        
        if self.do_concatenate: 
            res['img_concat'] = feature_image[:, :, self.n_main_channels:]
    ...
    def forward(self, planes, pose, neural_rendering_resolution=None, aggregation_mode='sum'):
        ...
        returns:
            images: torch.Tensor: Renderings with shape [bs, n_renders, total_channels, neural_rendering_resolution, neural_rendering_resolution]
        """
