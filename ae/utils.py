...
def render_all_poses_via_emulated_batches(triplane_renderer, poses, triplane, render_batch_size, latent_to_pil_transform, 
    ...
    for idx_tri, idx_pose in outter_loop() :
        ...
        for iidx_tri, iidx_pose in inner_loop(n_tri, n_ren) : 
                ...
                if 'img_concat' in out.keys() : 
                    imgs_dict['low_res_rgb'] = latent_to_pil_transform(out['img_concat'][iidx_tri, iidx_pose])
...
def compute_tv(t:torch.Tensor, mode="l2"):
    "Make the input smooth (l2) or sparse (l1) over the last two dimensions"
    if mode == "l2":
        positify = torch.square
    elif mode == "l1" : 
        positify = torch.abs
    else : 
        raise NotImplementedError(f"mode is {mode} but only 'l2' or 'l1' are supported")
...
def compute_kl_div(x_mu, x_sigma):
    return (x_sigma ** 2 + x_mu ** 2 - torch.log(x_sigma) - 0.5).sum()
